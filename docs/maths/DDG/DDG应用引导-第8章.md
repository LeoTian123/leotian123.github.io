...
4.8.4节提到了离散Hodge star的定义，仔细看了看似乎没说太细，而且没有谈到二维流形上的处理，这里结合资料谈一下。 $n$ 维Hodge star对应的那个矩阵记作 $\star_n$ ，是一个**对角矩阵**。既然对于 $n$ 维Hodge star作用在 $n$ 维的元素上，那么这东西的行列数量就是对应维度元素的数量，前三个维度就是 $V,E,F$ 。还有不要忘了书上这个定义：

$\star \alpha = \frac{|\sigma ^ \star|}{|\sigma|} \alpha$

那个 $\sigma$ 就是元素， $\sigma ^ \star$ 是对偶元素，双竖线是度量的意思。一个点的度量就是 $1$ 。

**（0）**对于 $\star _0$ ，这东西对应的矩阵对角元素是 $(\star _0)_{vv} = [毗邻面重心连线的面积]$ 。

右边这个我们给它限制在mesh表面（这其实可以看做一种度量选择，不同于 $d$ 的含义很确定，霍奇星导致的 $\delta$ 是度量诱导的），变为

$(\star _0)_{vv} = \frac{1}{3} \sum_{毗邻面}  [毗邻面的面积]$

**（1）**对于 $\star _1$ ，这东西对应的矩阵对角元素是 $(\star _1)_{ee} = \frac{[对偶边长]}{[本边长]}$ 。

【对偶边长】如果严格对应到cotan定理，应该写作 $\frac{1}{2}(cot\alpha+cot\beta)$ ，如果这样，通过霍奇星得到余切拉普拉斯矩阵（我们一直在用的拉普拉斯）就是可行的。

也可以进行工程近似，两个三角形重心连线，这东西我们知道是两个三角形中另外两个点的连线长的三分之一。

**（2）**对于 $\star _2$ ，这东西对应的矩阵对角元素是 $(\star _2)_{ff} = \frac{1}{[本三角形面积]}$ 。
...
$E-(V-1)-(F-1) = 2-\chi=2g$ 这 $2g$ 个边，每一个都对应了一个生成元，这个边加入到tree里，会得到唯一一个环路，生成元就是这个环路。
