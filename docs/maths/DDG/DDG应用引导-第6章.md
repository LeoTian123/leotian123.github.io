...
-   凸点，拉普拉斯向内，外法线向外，二者点积负数，所以染上蓝色
-   凹点，拉普拉斯向外，外法线向外，二者点积正数，所以染上蓝色

所以核心就是外法线怎么求。那么其实啥额外信息也不指定的话，这东西是没法求的，然而，大伙可以看一下下面的示意图，颜色十分成功，正如预期的那样。那么这就说明按照代码np.cross(v1 - v0, v2 - v0)的方法求得的正是外法线，这其实意味着，v0,v1,v2在从外往里的方向看，是正向的（逆时针的）。这是我拿到的数据很好的特点，如果不是这样，还需要人工做一个面的内外定向，然后用程序把所有其他面的定向整理好。
